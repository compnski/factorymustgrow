...
import { BeltConnection, EntityStack, NewEntityStack, NewMainBus, NewRegionFromInfo } from "./types"
...
} & Readonly<Pick<Building, "kind" | "subkind" | "ProducerType" | "BuildingCount">> &
...
export const initialFactoryGameState = () => ({
  RocketLaunchingAt: 0,
  Research: {
    Progress: ImmutableMap(startingResearch.map((research) => [research, NewEntityStack(research, 0, 0)])),
    CurrentResearchId: "",
  },
  ...
  Regions: ImmutableMap([["region0", NewRegionFromInfo(GetRegionInfo("region0"))]]),

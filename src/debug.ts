...
export class DebugInventory extends ReadonlyInventory {
  ...
  Entities(): [string, number][] {
    return availableItems(DebugResearch).map((entity) => [entity, GetEntity(entity).StackSize])
  }
}

export const DebugResearch = {
  Progress: ImmutableMap(
    AvailableResearchList.map(({ Id: research }) => [research, NewEntityStack(research, 0, 0)])
  ),
  CurrentResearchId: "",
}

...
export function findBelt(
  ...
  const beltIdx = belts.findIndex(
    (b) => b.laneIdx == laneId && b.upperSlotIdx <= buildingIdx && b.lowerSlotIdx >= buildingIdx
  )
...
export function PushPullFromMainBus(
  ...
  for (const [connectionIdx, laneConnection] of slot.BeltConnections.entries()) {
    ...
    if (laneConnection.Inserter.direction != "TO_BUS" && laneConnection.Inserter.direction != "FROM_BUS")
...
export function capacityAtBuildingIdx(belt: Belt, buildingIdx: number): number {
  return remainingCapacity(belt.internalBeltBuffer[buildingIdx - belt.upperSlotIdx])
}
...
export class BufferForMainBusLane {
  ...
  Count(entity: string): number {
    return this.Accepts(entity) ? countAtBuildingIdx(this.belt, this.buildingIdx) : 0
  }

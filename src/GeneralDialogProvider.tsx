...
export type GeneralDialogConfig = {
  actionCallback?: (arg0: { returnData: any | false; uxDispatch: (a: GameAction) => void }) => void
  title: string
  component?: (onConfirm: (evt: SyntheticEvent, returnData: any) => any) => JSX.Element
}
...
const GeneralDialogProviderWithShortcut = ({
  ...
  const [generalDialogConfig, setGeneralDialogConfig] = useState<GeneralDialogConfig>({
    title: "",
  })

  const openGeneralDialog = ({ title, component, actionCallback }: GeneralDialogConfig) => {
  ...
  const onConfirm = (evt: SyntheticEvent | KeyboardEvent | undefined, ...returnData: any) => {
    resetGeneralDialog()
    generalDialogConfig.actionCallback && generalDialogConfig.actionCallback({ returnData, uxDispatch })
  }

  const dialog = generalDialogConfig.component && generalDialogConfig.component(onConfirm)

  useEffect(() => {
    if (shortcut && shortcut.registerShortcut) {
      shortcut.registerShortcut(onConfirm, ["escape"], "Escape", "Cancel Action")
      return () => {
...
export const GeneralDialogProvider = withShortcut(GeneralDialogProviderWithShortcut)

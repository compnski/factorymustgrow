...
export class ResearchOutput implements ReadonlyItemBuffer {
  ...
  SetResearch(researchId: string, progress: number, maxProgress: number): ResearchOutput {
...
export function NewLab(initialProduceCount = 0): Lab {
  return {
    ...
    inputBuffers: ReadonlyFixedInventory(initialLabInput.map((input) => NewEntityStack(input.Entity, 0, 50))),
    RecipeId: "",
...
export function IsResearchComplete(researchState: ReadonlyResearchState): boolean {
  const researchProgress = researchState.Progress.get(researchState.CurrentResearchId)
  if (researchProgress) return researchProgress.Count >= (researchProgress.MaxCount || Infinity)
  return false
}

export function setLabResearch(lab: Lab, researchId: string, progress: number): Lab {
  const maxProgress = researchId ? GetResearch(researchId).ProductionRequiredForCompletion : 0
  return {
    ...lab,
    RecipeId: researchId,
    outputBuffers: (lab.outputBuffers as ResearchOutput).SetResearch(researchId, progress, maxProgress),
    progressTrackers: [],
  }
}
...
export function ResearchInLab(
  ...
  const currentResearchProgress = researchState.Progress.get(currentResearchId)?.Count || 0
  ...
  const remainingResearchProgress = Math.max(
    0,
    research.ProductionRequiredForCompletion - currentResearchProgress - existingProgressTrackerCount
  )
  ...
  if (!actualProduction && (l.outputBuffers as ResearchOutput).researchId) return 0
...
export function unlockedResearch(researchState: ReadonlyResearchState): string[] {
  ...
  for (const research of AvailableResearchList) {
    ...
    if ([...research.Prereqs].filter((x) => !unlockedResearch.has(x)).length === 0) {
...
export function availableRecipes(
  ...
  let availableRecipes = availableRecipesIds.filter((r) => !IgnoredRecipies.has(r)).map((r) => GetRecipe(r))
  ...
  availableRecipes.sort((recipeA, recipeB): number => {
    ...
    if (entA.Category !== entB.Category) return categoryOrder[entA.Category] - categoryOrder[entB.Category]

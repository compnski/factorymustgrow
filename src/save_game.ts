...
export async function loadCloudSaveGame(cloudSaveName: string, sgmKey: string): Promise<string | undefined> {
  ...
  if (data?.serializedGameState) return fromSaveGame(data as Pick<SaveGame, "serializedGameState">)
}
...
async function fetchCloudSaveState(
  ...
  const response = await fetch(getCloudSaveUrl + `?cloudSaveName=${cloudSaveName}&saveVersion=${saveVersion}`)
  if (!response.ok) {
    return
  }
...
export async function listSavedGamesInCloudStorage(cloudSaveName: string) {
  ...
  const response = await fetch(listCloudSaveUrl + `?cloudSaveName=${cloudSaveName}`)
  if (!response.ok) {
    return
  }
...
export async function saveGameToCloudStorage(cloudSaveName: string, gs: FactoryGameState, name: string) {
  if (!settings().cloudSaveEnabled) return
...
export async function deleteGameFromCloudStorage(cloudSaveName: string, saveVersion: string) {
  if (!settings().cloudSaveEnabled) return
  await fetch(saveCloudSaveUrl + `?cloudSaveName=${cloudSaveName}&saveVersion=${saveVersion}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function saveGameToLocalStorage(gs: FactoryGameState, name: string) {
...
async function toSaveGame(gs: FactoryGameState, saveVersion: string): Promise<SaveGame> {

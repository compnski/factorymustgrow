...
export function AddToEntityStack(stack: EntityStack, count: number): EntityStack {
...
export function NewEntityStack(e: string | Entity, initialCount = 0, maxCount = 0): EntityStack {
...
export function MergeEntityStacks(...stacks: EntityStack[]) {
  ...
  stacks.forEach(({ Entity, Count }) => (counts[Entity] = (counts[Entity] || 0) + Count))
...
export function NewRegionFromInfo(info: RegionInfo): Region {
  return NewRegion(info.Id, info.LaneCount, info.LaneSize, info.MainBusCount, [...info.Provides])
}

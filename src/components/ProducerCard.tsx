import { ReadonlyBuilding, ReadonlyItemBuffer, ReadonlyResearchState } from "../factoryGameState"
...
function formatRecipeName(s: string | undefined): string {
  ...
  const title = s.split("-").map((s) => s.charAt(0).toUpperCase() + s.slice(1).toLowerCase())
    ...
    <div className="main-area">
      <div className="top-area">
        <div
          ...
          title="Change Recipe"
        >
          <span className="title-text">{formatRecipeName(producer.RecipeId) || "No Recipe Selected"}</span>

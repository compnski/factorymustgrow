...
function buildHelpRegion() {
  ...
  const regionInfo = GetRegionInfo("HelpRegion"),
    ...
    miner = NewExtractorForRecipe({ subkind: "burner-mining-drill" }, "iron-ore", 1),
    smelter = NewFactoryForRecipe({ subkind: "stone-furnace" }, "iron-plate", 1),
    assembler = NewFactoryForRecipe({ subkind: "assembling-machine-1" }, "iron-gear-wheel", 1),
    assembler2 = NewFactoryForRecipe({ subkind: "assembling-machine-1" }, "transport-belt", 1)

  miner.outputBuffers = miner.outputBuffers.AddItems(miner.outputBuffers.Entities()[0][0], 10)
  smelter.outputBuffers = smelter.outputBuffers.AddItems(smelter.outputBuffers.Entities()[0][0], 10)
  assembler.outputBuffers = assembler.outputBuffers.AddItems(assembler.outputBuffers.Entities()[0][0], 10)
      ...
      <div className="inner-frame">
        ...
        <div className="help-text">
          ...
          <div style={{ fontSize: 16 }}>
            Build a lab to research technologies and expand what your factory can produce.
          </div>
        </div>

import { ReadonlyItemBuffer, ReadonlyRegion, ReadonlyResearchState } from "../factoryGameState"
...
export async function showMoveItemToFromInventorySelector(
  ...
  const { selected, uxDispatch } = await showIconSelector(showDialog, "Add Stack", items)
...
export async function showSetLaneEntitySelector(
  ...
  const { selected: item, uxDispatch } = await showIconSelector(showDialog, "Choose Item Filter", items)
  if (item && item != "") {
    uxDispatch({
      type: "SetLaneEntity",
      entity: item,
...
export async function showChangeProducerRecipeSelector(
  ...
  const { selected: recipeId, uxDispatch } = await showIconSelector(
    showDialog,
    "Choose Recipe",
    availableRecipes(researchState).filter(function filterRecipes(recipeId: string): boolean {
      const r = GetRecipe(recipeId)
      return r.ProducerType === producerType
    })
  )
  if (recipeId) uxDispatch({ type: "ChangeRecipe", buildingIdx, regionId, recipeId })
}
...
export async function showPlaceBuildingSelector(
  ...
  const { selected: item, uxDispatch } = await showIconSelector(showDialog, "Choose Building", [
    ...new Set(
      inventory
        .Entities()
        .map((s) => s[0])
  ...
  if (item === "concrete") {
    await showPlaceTruckLineSelector(showDialog, inventory, regions, regionId, buildingIdx)
  } else if (item)
    uxDispatch({
      type: "PlaceBuilding",
      entity: item,
...
export async function showResearchSelector(
  ...
  const result = await showDialog({
    ...
    component: (onConfirm) => <SelectResearchPanel onConfirm={onConfirm} researchState={researchState} />,
  })
...
export async function showClaimRegionSelector(
  ...
  const result = await showDialog({
    title: "Claim Region",
    component: (onConfirm) => (
      <RegionSelector inventory={inventory} regionIds={regionIds} onConfirm={onConfirm} />
    ),
  })
  if (result.returnData) {
    const {
...
export async function showSaveCard(
  showDialog: (c: GeneralDialogConfig) => Promise<{
    returnData: false | (string | { saveVersion: string; cloudSaveName: string | undefined })[]
    uxDispatch: (a: GameAction) => void
  }>
): Promise<void> {
  const { returnData: stateToLoad, uxDispatch } = await showDialog({
    title: "Save",
    component: (onConfirm) => <SaveCard onConfirm={onConfirm} showSaveButton={true} />,
  })

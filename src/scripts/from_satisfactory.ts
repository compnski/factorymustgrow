...
export function convertToEntity(filename: string) {
  ...
  const data = JSON.parse(contents.toString("utf-8")) as Record<
    string,
    SatisfactoryItem
  >;
  ...
  return Object.values(data)
    .map((recipe: SatisfactoryResearch) => {
      ...
      if (r.Unlocks.length == 0) {
        if (recipe.type != "EST_ResourceSink")
          console.log("Useless recipe ", recipe.name, recipe.className);
        return null;
      }
      return [r.Id, r];
    })

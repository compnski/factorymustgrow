import { AddProgressTrackers, RemoveProgressTracker, TickProgressTracker } from "./AddProgressTracker"
...
describe("Progress Trackers", () => {
  ...
  describe("Ticks", () => {
    it("Does nothing when there are no expiring trackers", () => {
      ...
      expect(TickProgressTracker(vmDispatch, address, t, 5, 5, Infinity)).toBe(0)
      expect(vmDispatch).not.toHaveBeenCalled()
    })

    it("Returns a count of expiring trackers and removes them", () => {
      const t = tracker(3, [1, 2, 3])
      expect(TickProgressTracker(vmDispatch, address, t, 7, 5, Infinity)).toBe(2)
      expectTracker(-2, 7)
    })

...
describe("Factories", () => {
  ...
  it("Produces a single item", () => {
    ...
    TestFactory(factory, {
      ...
      inputBuffers: [NewEntityStack("test-ore", -2), NewEntityStack("test-slow-ore", -3)],
    })
  })
  ...
  it("Produces three item", () => {
    ...
    TestFactory(factory, {
      ...
      inputBuffers: [NewEntityStack("test-ore", -6), NewEntityStack("test-slow-ore", -9)],
    })
  })
...
describe("Extractors", () => {
  ...
  it("Won't overfill inventory", () => {
    const extractor = NewTestExtractor("test-ore", 5)
    const region = NewTestRegion([NewEntityStack("test-ore", 3)])
    extractor.outputBuffers = AddItemsToFixedBuffer(extractor.outputBuffers, 48)

...
export function ReadonlyFixedInventory(slotTemplate: EntityStack[]): ReadonlyInventory {
  const slots = ImmutableMap(slotTemplate.map((stack: EntityStack) => [stack.Entity, stack.Count]))
...
export class ReadonlyInventory implements ReadonlyItemBuffer {
  ...
  constructor(maxCapacity: number, Data?: ImmutableMap<string, number>, immutableSlots = false) {
  ...
  AddItems(entity: string, count: number, canExceedCapacity = false): ReadonlyInventory {
    if (count < 0) {
  ...
  private RemoveItems(entity: string, count: number): ReadonlyInventory {
    ...
    const newData =
      this.immutableSlots || newCount > 0 ? this.Data.set(entity, newCount) : this.Data.delete(entity)
  ...
  AvailableSpace(entity: string): number {
    ...
    if (this.HasSlotFor(entity))
      return availableSlots * stackSize + (count < stackSize ? stackSize - (count % stackSize) : 0)

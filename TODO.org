...
Consume resources to start production
For each set of consumed resources, add a 'tick counter' that counts up. When it reaches ticks to build, output resources
On destruction, refund input cost * outstanding tick counters
